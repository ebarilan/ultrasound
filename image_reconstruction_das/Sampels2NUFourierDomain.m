...
fxSNonUniformClean2 = fxSNonUniformClean + fx0;
% figure
% scatter(fxSNonUniformClean2(:), fSNonUniformCleanRF);

if theta <= 0
    indWrapInverse = (fxSNonUniformClean2 < -fsx/2);
    fxSNonUniformClean2(indWrapInverse) = fxSNonUniformClean2(indWrapInverse) + fsx;
%     figure
%     scatter(fxSNonUniformClean2(:), fSNonUniformCleanBB(:) + BW_f);
else
    indWrapInverse = (fxSNonUniformClean2 > fsx/2);
    fxSNonUniformClean2(indWrapInverse) = fxSNonUniformClean2(indWrapInverse) - fsx;
%     figure
...
s_phase = ifft(ifftshift(S_f_x.*expPhase,1),[],1);

s = s_phase;
omX = fxSNonUniformClean2   * (2*pi/fsx);
...
% figure
% scatter(fxSNonUniformClean2, fSNonUniformCleanBB+BW_f, [] , abs(S_NU))


[fx_mesh, fz_mesh, Gamma, fx, fsx] = FromS2Gamma(fxSNonUniformClean2 ,fSNonUniformCleanBB+BW_f, S_NU, dataset, probeIdx);
image3 = InterpNUFFT(Gamma, scan, fz_mesh, fx_mesh , fsx, 0);
figure, imagesc(db(abs(image3)))

imageRecover = abs(image3);
