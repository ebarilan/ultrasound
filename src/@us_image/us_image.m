classdef us_image < handle
    ...
    methods (Access = public)
        ...
        function [im, fig] = show(h,dynamic_range,frame_list)
            ...
            fig = figure; set(gca,'fontsize',16);
    ...
    methods (Access = public)
        ...
        function [im, fig] = showPart(h,dynamic_range,frame_list,indNum)
            ...
            part = indNum/size(h.scan.z_matrix,1);
            
            %-- setting axis limits (mm)
            x_lim = [min(h.scan.x_matrix(:)) max(h.scan.x_matrix(:))]*1e3; 
            z_lim = [min(h.scan.z_matrix(:)) part * max(h.scan.z_matrix(:))]*1e3; 
            ...
            for f=frame_list
                    ...
                    %-- compute dB values
                    env = h.data(1:indNum,:,f);
                    im = 20*log10(env./max(env(:)));
                    vrange = [-dynamic_range 0];

                    %-- display image
                    imagesc((h.scan.x_axis)*1e3,(h.scan.z_axis(1:indNum))*1e3,im); 
                    shading flat; colormap gray; caxis(vrange); colorbar; hold on;
                    axis equal manual;
                    xlabel('x [mm]');
                    ylabel('z [mm]');
